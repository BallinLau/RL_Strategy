#!/usr/bin/env python3
"""
通用回测脚本 - 支持任意 Episode 模型回测

用法:
    python3 scripts/run_backtest.py --episode 10
    python3 scripts/run_backtest.py --episode 20
    python3 scripts/run_backtest.py --model-path /path/to/model.pth --output-dir backtest/custom
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='运行模型回测')
...
def main():
    ...
    if args.model_path:
        ...
        if args.output_dir:
            ...
            if match:
                output_dir = f"backtest/ep{match.group(1)}"
            else:
                output_dir = "backtest/custom"
        ...
        output_dir = args.output_dir or f"backtest/ep{args.episode}"
    else:
        print("错误: 请指定 --episode 或 --model-path")
        print("\n示例:")
        print("  python3 scripts/run_backtest.py --episode 10")
        print("  python3 scripts/run_backtest.py --model-path /path/to/model.pth")
        sys.exit(1)
    
    config_path = args.config_path
    
    print("="*80)
    print(f"Episode {args.episode if args.episode else 'Custom'} 模型回测")
    print("="*80)
    ...
    with open(report_path, "w") as f:
        f.write("="*80 + "\n")
        f.write(f"Episode {args.episode if args.episode else 'Custom'} 回测报告\n")
        f.write("="*80 + "\n\n")
